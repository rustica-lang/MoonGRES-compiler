...
module Key = struct
  ...
  include struct
    ...
    let equal =
      (fun a__001_ -> fun b__002_ -> Stdlib.( = ) (a__001_ : int) b__002_
        : t -> t -> bool)

    let _ = equal

    let compare =
      (fun a__003_ -> fun b__004_ -> Stdlib.compare (a__003_ : int) b__004_
        : t -> t -> int)
...
let of_array xs = Arr.fold_left xs empty (fun acc -> fun (k, v) -> add acc k v)
