...
module Key = struct
  ...
  let to_string (x : t) =
    match x with
    | Pident { name; stamp } ->
        (name ^ "/" ^ Int.to_string stamp : Stdlib.String.t) [@merlin.hide]
    ...
    | Plocal_method { index = _; name_; trait; method_name } ->
        ...
        (Stdlib.String.concat ""
           [ trait_str; "::"; method_name; "|"; name_; "|" ] [@merlin.hide])
...
let of_qual_ident (x : Qual_ident.t) = (Pdot x : t)
...
let as_export_name (x : t) =
  ...
  | Pdot (Qregular { pkg; name }) | Pdot (Qregular_implicit_pkg { pkg; name })
    -> (
      match pkg with
      | "" -> name
      | _ -> (pkg ^ "::" ^ name : Stdlib.String.t) [@merlin.hide])
